    ...
    async def connect(self):
        ...
        while self.running:
            try:
                ...
                logging.warning(f"WS disconnected: {e}. Reconnecting in 3s...")
...
# ==================== DASHBOARD (from Bot 2, full version) ====================
...
def build_dashboard(bot) -> dict:
    ...
    for p in bot.positions.values():
        # Prefer WebSocket price, then position's cached current_price, then entry
...
class PolymarketExecutor:
    ...
    def is_order_filled(self, order_id: str) -> Optional[bool]:
        """
        Returns True (filled), False (confirmed open/unfilled), or
                ...
                if min_price > 0:
                    try:
                        market_args = MarketOrderArgs(
                            token_id=token_id, amount=shares, side=Side.SELL,
...
class CopyTrader:
        ...
        for pos_key, pending in list(self.pending.items()):
            ...
            if filled is None:
                ...
                if pending.fill_check_errors >= MAX_FILL_CHECK_ERRORS:
                    logging.error(
                        f"Max fill check errors reached for {pending.question[:40]} — cancelling"
                    )
                    self.executor.cancel_order(pending.order_id)
                    del self.pending[pos_key]
                continue
            ...
            if age >= LIMIT_EXPIRY_SECONDS:
                ...
                if ok:
                    ...
                    logging.info(
                        f"LIMIT BUY RETRIED | {pending.question[:40]} @ {actual_price:.4f}"
                    )
    ...
    def _execute_sell(
        ...
        if self.dry_run:
            # Use WS price first, then cached current_price, then entry
            ...
            if ok:
                contamination = sum(
                    cost for pk, cost in pending_costs_before.items()
    ...
    async def scan_and_copy(self):
        ...
        for wallet_addr, config in WALLETS.items():
            ...
            if wallet_addr not in self._first_scan_done:
                ...
                if copy_mode == "new_only":
                    ...
                    logging.info(
                        ...
                        f"at deployment, will copy unseen ones now"
                    )
            ...
            for pos in raw:
                ...
                # Prefer WS price, fall back to API price
            ...
            # Update current prices from REST API for positions (WS is primary, this is fallback)
            cur_price_map = {
