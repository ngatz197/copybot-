...
class OrderbookCache:
    ...
    def get(self, token_id: str) -> Optional[Tuple[float, float, float]]:
...
def build_dashboard(bot) -> dict:
    ...
    for p in active_positions:
        ...
        <tr>
            ...
            <td class="price-mono">{entry_cents_str}</td>
            <td class="price-mono">{cur_cents_str}</td>
    ...
    for p in reversed(closed_list):
        ...
        <tr>
            ...
            <td><span class="outcome-pill {outcome_cls}">{p.outcome}</span></td>
            <td class="price-mono">{entry_cents_str}</td>
            <td class="price-mono">{exit_cents_str}</td>
        ...
        if self.path == "/" and _bot_ref:
            ...
            if send_body:
                try:
                    ...
                    logging.error(f"Dashboard processing error: {e}")
...
if __name__ == "__main__":
    ...
    # Keep main alive for integration verification loop tests
    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        logging.info("Shutting down engine structures smoothly.")
