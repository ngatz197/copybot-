...
PER-WALLET COPY RULES:
  ...
  Wallet903  (0xf903...): only copy NEW trades appearing after deployment,
                          value >= $1, 20% ask cap.
  WalletE8ca (0xe8ca...): only copy NEW trades appearing after deployment,
                          value >= $1, 20% ask cap.
        ...
        if force or self.cached_balance is None or (time.time() - self.last_update > 30):
            ...
            if real > 0:
                ...
                if self.cached_balance is None:
                    logging.error("Could not fetch real pUSD balance — bot will not trade until balance is confirmed")
...
class PolymarketExecutor:
    ...
    def place_limit_buy(self, token_id: str, amount_usd: float, limit_price: float) -> Tuple[bool, str, float]:
        ...
        if self.dry_run or self.client is None:
            logging.info(f"[DRY RUN] LIMIT BUY {shares:.4f} shares @ {limit_price:.4f} (${amount_usd:.2f}) token {token_id[:12]}…")
...
class CopyTrader:
    ...
    # ---- PENDING LIMIT ORDER MANAGEMENT (RESTORED & COMPLETE) ----
    def _process_pending_orders(self, source_token_ids_by_wallet: Dict[str, set]):
        for pos_key, pending in list(self.pending.items()):
            ...
            # 1. Source wallet exited before we filled — cancel order immediately
            ...
            # 3. Expired — cancel and retry execution loop with an adjusted ask cap
            ...
            if age >= LIMIT_EXPIRY_SECONDS:
                ...
                if ok:
                    ...
                    logging.info(f"LIMIT BUY RETRIED (V2) | {pending.question[:40]} @ {filled_price:.4f} (ask={current_ask:.4f})")
    ...
    async def scan_and_copy(self):
        ...
        logging.info(
            f"Scanning | bankroll=${current_bankroll:.2f} pUSD | compounding=${compounding_bankroll:.2f} | "
            f"open={len(self.positions)} | pending={len(self.pending)} | seen={len(self.seen._seen)}"
        )
        ...
        for wallet_addr, config in WALLETS.items():
            ...
            if wallet_addr not in self._first_scan_done:
                ...
                if copy_mode == "new_only":
                    ...
                    logging.info(f"[{name}] new_only — {len(all_keys)} existing position(s) snapshotted at deployment, skipping buy loop this scan")
                    source_token_ids_by_wallet[wallet_addr] = source_token_ids
                    continue

                else:
                    logging.info(f"[{name}] copy_all — {len(source_token_ids)} position(s) open at deployment, will copy unseen ones now")

            logging.info(f"[{name}] {len(raw)} position(s) from API, {len(source_token_ids)} with active tokens")
            ...
            for pos in raw:
                ...
                logging.info(f"[{name}] {question[:40]} | seen={already_seen} open={in_positions} pending={in_pending} val=${size_usd:.2f}")
                ...
                logging.info(f"[{name}] curPrice={cur_price:.4f} cap={price_cap:.4f} | {question[:40]}")
                ...
                if ok:
                    ...
                    logging.info(f"[{name}] LIMIT BUY PLACED (V2) | {question[:40]} | ${my_size:.2f} @ {actual_price:.4f}")
                ...
                if position.token_id not in source_token_ids and position.status == "open":
                    ...
                    if ok:
                        ...
                        if pnl > 0:
                            ...
                            logging.info(f"Compounding profit: +${pnl * COMPOUNDING_RATE:.4f} → compounding_bankroll=${compounding_bankroll:.2f}")

                        logging.info(f"[{name}] MARKET SELL (V2) | {position.question[:40]} | exit={exit_price:.4f} pnl=${pnl:.2f}")
        ...
        while True:
            ...
            if now - last_heartbeat >= 300:
                ...
                logging.info(
                    ...
                    f"compounding=${compounding_bankroll:.2f} | open={len(self.positions)} | "
                    f"pending={len(self.pending)} | seen={len(self.seen._seen)} | storage={self.seen.backend}"
                )
                last_heartbeat = now
