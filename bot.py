    ...
    async def scan_and_copy(self):
        ...
        for wallet_addr, config in WALLETS.items():
            ...
            for pos in raw:
                ...
                success, order_id, _ = self.executor.place_limit_buy(token_id, position_size, limit_price)
                ...
                if success:
                    pending = PendingLimitBuy(
                        ...
                        order_id=order_id,
                        source_shares=float(pos.get("shares", 0))
                    )
                    self.pending[pos_key] = pending
                    self.seen.mark_seen(pos_key)
                    logging.info(f"📝 Placed limit order: {config['name']} -> ${position_size:.2f} @ {limit_price:.4f}")
