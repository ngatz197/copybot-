...
class PolymarketExecutor:
    ...
    def estimate_vwap_fill(self, book: dict, shares_needed: float) -> float:
        asks = book.get("asks", [])
    ...
    def place_aggressive_buy(self, token_id:str, amount_usd:float, max_price:float):
        if self.dry_run or self.client is None:
            return self.place_limit_buy(token_id, amount_usd, max_price)
        shares=round(amount_usd/max_price,4)
        try:
            result=self.client.create_and_post_market_order(order_args=MarketOrderArgs(token_id=token_id, amount=shares, side=Side.BUY), options=PartialCreateOrderOptions(tick_size="0.01"), order_type=OrderType.FOK)
            oid=result.get("orderID", result.get("id","unknown"))
            return True, oid, max_price
        except Exception:
            return self.place_limit_buy(token_id, amount_usd, max_price)
