...
class CopyTrader:
    ...
    async def _on_ws_order_placed(self, ev: dict):
        ...
        async with self._pending_lock:
            ...
            ok, order_id, _ = self.executor.place_aggressive_buy(token_id, my_size, actual_price)
            if not ok:
                logging.warning(
    ...
    async def _on_own_fill(self, ev: dict):
        ...
        self.positions[matched_key] = Position(
            ...
            source_shares = 0.0,
            source_entry_price = signal_price if "signal_price" in locals() else 0.0,
            source_timestamp = time.time(),
            entry_slippage = (price if price > 0 else p.limit_price) - (signal_price if "signal_price" in locals() else 0.0),
        )
    ...
    def get_cached_orderbook_prices(self, token_id:str):
        v=self._orderbook_cache.get(token_id)
        if v: return v
        v=self.get_orderbook_prices(token_id)
        self._orderbook_cache[token_id]=v
        return v
    ...
    def process_pending_fills(self):
        for k, p in list(self.pending.items()):
            ...
            source_entry_price = signal_price if "signal_price" in locals() else 0.0,
            source_timestamp = time.time(),
            entry_slippage = (price if price > 0 else p.limit_price) - (signal_price if "signal_price" in locals() else 0.0),   # populated on the next REST poll
                )
                del self.pending[k]
